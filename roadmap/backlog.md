...
- [ ] Keep `cargo test --manifest-path harnesses/rust-harness/Cargo.toml` green.
- [ ] Keep `bash tests/run.sh` green on macOS and Linux without global Python package installs.
- [ ] Preserve the canonical runner: `scripts/harness.sh --repo <path> --time <duration> --executor openclaw`.
- [ ] Maintain stable JSONL event names with `run_id`, `task_id`, and monotonic `seq`.
- [ ] Keep `scripts/captain-doctor.sh` as the first troubleshooting command.
...
- [ ] Record the 90-second demo from `demo/90-second-demo.md`.
