...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage:
  captain/harnesses/rust-harness/scripts/events-pretty.sh [events_file] [--format plain|emoji]

Examples:
  captain/harnesses/rust-harness/scripts/events-pretty.sh
  captain/harnesses/rust-harness/scripts/events-pretty.sh --format emoji
  captain/harnesses/rust-harness/scripts/events-pretty.sh /path/to/events.jsonl --format plain
EOF
      exit 0 ;;
    *)
