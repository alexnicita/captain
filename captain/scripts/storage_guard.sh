...
report() {
  ...
  du -sh "$HARNESS_DIR/target" 2>/dev/null | sed 's/^/[storage-guard] /' || true
  du -sh "$HARNESS_DIR/runs" 2>/dev/null | sed 's/^/[storage-guard] /' || true
}
...
prune_light() {
  ...
  find "$HARNESS_DIR/runs" -type f -mtime +14 -delete 2>/dev/null || true
}

prune_aggressive() {
  log "aggressive prune: cleaning Rust build caches"
  (cd "$HARNESS_DIR" && cargo clean) || true
